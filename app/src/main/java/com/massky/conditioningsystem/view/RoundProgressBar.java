...
public class RoundProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setStrokeWidth((radius - radius_innner) / 2); //设置圆环的宽度
...
//        float textWidth = paint.measureText(percent + "%");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间
        float textWidth = paint.measureText(max - progress + "s");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间
        if (textIsDisplayable && max - progress != 0 && style == STROKE) {
            canvas.drawText(max - progress + "s", centre - textWidth / 2, centre + textSize / 2, paint); //画出进度百分比
        }
        ...
        RectF oval = new RectF(centre - radius_innner, centre - radius_innner, centre
                + radius_innner, centre + radius_innner);  //用于定义的圆弧的形状和大小的界限
